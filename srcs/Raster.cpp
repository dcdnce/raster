...
void	Raster::drawTriangle(Vector2 p1, Vector2 p2, Vector2 p3, Color color) {
	...
	int	B_height = p2.y - p1.y + 1;
	// For each y -> find x
	for (float y = p1.y ; y < p2.y ; y++) {
		float	A_scalar = (y - p1.y) / A_height; // Actual y division by max y
		float	B_scalar = (y - p1.y) / B_height;
		Vector2 A = (Vector2){0, y};
		Vector2 B = (Vector2){0, y};
		A.x = p1.x + (p3.x - p1.x) * A_scalar; // Interpolation
		B.x = p1.x + (p2.x - p1.x) * B_scalar;
		DrawLineV(A, B, color);
	}

	B_height = p3.y - p2.y + 1;
	for (float y = p2.y ; y < p3.y ; y++) {
		float	A_scalar = (y - p1.y) / A_height;
		float	B_scalar = (y - p2.y) / B_height;
		Vector2 A = (Vector2){0, y};
		Vector2 B = (Vector2){0, y};
		A.x = p1.x + (p3.x - p1.x) * A_scalar;
		B.x = p2.x + (p3.x - p2.x) * B_scalar;
		DrawLineV(A, B, color);
	}
}
